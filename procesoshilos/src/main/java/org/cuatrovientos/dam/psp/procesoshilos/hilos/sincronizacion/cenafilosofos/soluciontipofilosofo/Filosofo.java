...
public class Filosofo implements Runnable{
	...
	public Filosofo(String nombre, Tenedor tenedorDrcha, Tenedor tenedorIzq, TipoFilosofo tipo) {
		super();
		this.nombre = nombre;
	...
	private void sientaAComer() throws InterruptedException {
		...
		// Primero tiene que coger los tenedores, dependiendo del tipo
		if (this.tipo.equals(TipoFilosofo.DIESTRO)) {
			...
			tenedorIzq.cogerTenedor();
			logConTimestamp("El filosofo ha cogido el tenedor Izquierdo: "+tenedorIzq);
			tenedorDrcha.cogerTenedor();
			logConTimestamp("El filosofo ha cogido el tenedor Derecho y empieza a comer: "+tenedorDrcha);
		}
		
		// Come porque ya esta listo
		Thread.sleep((long) (Math.random() * (MAX_MILLIS_COMIENDO - MIN_MILLIS_COMIENDO) + MIN_MILLIS_COMIENDO)); 
		
		// Libera los tenedores, dependiendo del tipo
		if (this.tipo.equals(TipoFilosofo.DIESTRO)) {
